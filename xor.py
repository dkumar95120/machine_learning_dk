...
def EvalNetwork(inputValues, network):
    ...
    # YOUR CODE HERE
    # append 1 for the bias in the input vector
    x = np.append(inputValues,1)
    x1 = expit(np.dot(network[0][0], x))
    x2 = expit(np.dot(network[0][1], x))
    h = [x1, x2, 1]
    y = expit(np.dot(network[1], h))
    OutputValue = round(y)
